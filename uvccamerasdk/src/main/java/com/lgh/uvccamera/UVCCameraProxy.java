...
public class UVCCameraProxy implements IUVCCamera {
    ...
    private  boolean state=false; //是否拍照成功
    private String path;
    ...
    public  String  getUsb(){
        //获取当前所有的配置信息
        String dirName = mConfig.getDirName();
        return dirName;
    }
    /**
     * 注册usb插拔监听广播
    ...
    public String checkDevice() {
        UsbDevice usbDevice = mUsbMonitor.checkDevice();
        if(usbDevice!=null){
    ...
    public void savePicture(final byte[] yuv, final int width, final int height, final float rotation) {
        ...
        mSubscriptions.add(Observable.create(new Observable.OnSubscribe<String>() {
            ...
            public void call(Subscriber<? super String> subscriber) {
                ...
                Log.e("得到的file的地址",file.getName());
                path = FileUtil.saveYuv2Jpeg(file, yuv, width, height, rotation);
                //暂且判断是否成功
     ...
     * 拍照
     *
     * @param pictureName 只是获取图片的名称
     *
     * @return
     */
    @Override
    public boolean takePicture(String pictureName) {
        isTakePhoto = true;
        mPictureName = pictureName;
        Log.e("得到的名称",mPictureName);
        return isTakePhoto;
    }
    /**
     * 设置usb设备连接回调
